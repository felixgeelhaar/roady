...
func TestScanCodebaseTree(t *testing.T) {
	...
	_ = os.MkdirAll(filepath.Join(dir, "pkg", "domain"), 0755)
	_ = os.WriteFile(filepath.Join(dir, "main.go"), []byte("package main"), 0644)
	_ = os.WriteFile(filepath.Join(dir, "pkg", "domain", "model.go"), []byte("package domain"), 0644)
	...
	_ = os.WriteFile(filepath.Join(dir, ".git", "config"), []byte(""), 0644)
...
func TestScanCodebaseTree_Truncation(t *testing.T) {
	...
	for i := 0; i < 10; i++ {
		_ = os.WriteFile(filepath.Join(dir, "file"+string(rune('a'+i))+".go"), []byte("package x"), 0644)
	}
