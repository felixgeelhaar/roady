...
func (cr *CostReport) CSV() string {
	...
	if hasTax {
		lines := []string{"Task ID,Title,Rate,Hours,Rate Amount,Cost,Tax,Total"}
		for _, e := range cr.Entries {
			lines = append(lines, fmt.Sprintf("%s,%s,%s,%.2f,%.2f,%.2f,%.2f,%.2f",
				e.TaskID, e.Title, e.RateName, e.Hours, e.HourlyRate, e.Cost, e.Tax, e.TotalWithTax))
		}
		lines = append(lines, fmt.Sprintf("TOTAL,,,%.2f,,%.2f,%.2f,%.2f", cr.TotalHours, cr.TotalCost, cr.TotalTax, cr.TotalWithTax))
		return strings.Join(lines, "\n")
	}
	lines := []string{"Task ID,Title,Rate,Hours,Rate Amount,Cost"}
	for _, e := range cr.Entries {
		lines = append(lines, fmt.Sprintf("%s,%s,%s,%.2f,%.2f,%.2f",
			e.TaskID, e.Title, e.RateName, e.Hours, e.HourlyRate, e.Cost))
	}
	lines = append(lines, fmt.Sprintf("TOTAL,,,%.2f,,%.2f", cr.TotalHours, cr.TotalCost))
	return strings.Join(lines, "\n")
}
